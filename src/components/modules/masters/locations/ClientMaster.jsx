...
import { fetchClients, deleteClient, updateClient } from '../../../../store/slices/masterSlice'
...
const ClientMaster = () => {
  ...
  const { clients } = useSelector(state => state.master)
  const loading = clients?.loading || false
  const clientData = Array.isArray(clients?.data) ? clients.data : []
  ...
  const handleDelete = (record) => {
    confirm({
      ...
      onOk: async () => {
        try {
          await dispatch(deleteClient(record.id)).unwrap()
          message.success('Client deleted successfully')
          dispatch(fetchClients({ page: pagination?.page || 1, limit: pagination?.limit || 10 }))
        } catch (error) {
  ...
  const handleToggleStatus = (record) => {
    ...
    confirm({
      ...
      content: (
        <div>
          <p>Are you sure you want to {actionText} <strong>"{record.client_name}"</strong>?</p>
          {!newStatus && (
            <p style={{ color: '#ff4d4f', fontSize: '14px' }}>
              ⚠️ Deactivating this client will make it unavailable for new selections.
            </p>
          )}
        </div>
      ),
      ...
      onOk: async () => {
        try {
          ...
          message.success(`Client ${actionText}d successfully`)
          dispatch(fetchClients({ 
          ...
          message.error(error.message || `Failed to ${actionText} client`)
        }
      }
    })
  }
  ...
  const total = clients?.total || clientData.length
