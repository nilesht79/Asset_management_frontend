...
import { UserOutlined, LockOutlined, ToolOutlined, ArrowLeftOutlined } from '@ant-design/icons'
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (result.user.role === 'engineer') {
        ...
        setError('Invalid role. This login is specifically for Engineers.')
      }
    } catch (err) {
        ...
        <div className="text-center mb-8">
          ...
          <div className="mb-6">
            <div className="w-20 h-20 mx-auto bg-gradient-to-br from-red-500 to-red-700 rounded-3xl flex items-center justify-center shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-300">
              <ToolOutlined className="text-4xl text-white" style={{ filter: 'drop-shadow(0 4px 8px rgba(0,0,0,0.3))' }} />
            </div>
          </div>
          ...
          <Text className="text-red-100 text-lg font-light">
            Technical support and <span className="text-red-300 font-semibold">maintenance</span> systems
          </Text>
        </div>
