...
import './DepartmentHeadApprovals.css';
...
const DepartmentHeadApprovals = () => {
  ...
  useEffect(() => {
    if (pendingApprovals.length > 0) {
      const newStats = {
        pending: pendingApprovals.filter(r => r.status === 'pending_dept_head').length,
  ...
  const loadApprovals = () => {
    ...
    if (activeTab === 'pending') {
      params.status = 'pending_dept_head';
    }
    ...
    dispatch(fetchPendingDeptApprovals(params));
  };
  ...
  const handlePageChange = (page, pageSize) => {
    dispatch(fetchPendingDeptApprovals({
      page,
      limit: pageSize,
      ...filters,
      status: activeTab === 'pending' ? 'pending_dept_head' : undefined
    }));
  };
  ...
  const handleApprovalSubmit = async (values) => {
    try {
      ...
      if (approvalAction === 'approve') {
        await dispatch(approveDeptHead(payload)).unwrap();
        message.success('Requisition approved successfully');
      } else {
        await dispatch(rejectDeptHead(payload)).unwrap();
  ...
  return (
    <div className="dept-head-approvals-page">
      {/* Header */}
